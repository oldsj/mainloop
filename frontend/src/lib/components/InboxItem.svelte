<script lang="ts">
  ...
  const typeIcons: Record<string, string> = {
    ...
    notification: 'M14.857 17.082a23.848 23.848 0 0 0 5.454-1.31A8.967 8.967 0 0 1 18 9.75V9A6 6 0 0 0 6 9v.75a8.967 8.967 0 0 1-2.312 6.022c1.733.64 3.56 1.085 5.455 1.31m5.714 0a24.255 24.255 0 0 1-5.714 0m5.714 0a3 3 0 1 1-5.714 0',
    routing_suggestion: 'M7.5 21 3 16.5m0 0L7.5 12M3 16.5h13.5m0-13.5L21 7.5m0 0L16.5 12M21 7.5H7.5',
    feedback_addressed: 'M9 12.75 11.25 15 15 9.75M21 12a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z',
    review: 'M2.036 12.322a1.012 1.012 0 0 1 0-.639C3.423 7.51 7.36 4.5 12 4.5c4.638 0 8.573 3.007 9.963 7.178.07.207.07.431 0 .639C20.577 16.49 16.64 19.5 12 19.5c-4.638 0-8.573-3.007-9.963-7.178Z M15 12a3 3 0 1 1-6 0 3 3 0 0 1 6 0Z'
  };
...
<div
  class="border-l-4 p-4 transition-colors {priorityColors[item.priority]} {item.read_at ? 'opacity-75' : ''}"
>
  <div class="flex items-start gap-3">
    <svg
      ...
      stroke-width="1.5"
      stroke="currentColor"
      class="mt-0.5 h-5 w-5 shrink-0 text-neutral-600"
    >
    ...
    <div class="min-w-0 flex-1">
      <div class="flex items-center justify-between gap-2">
        ...
        <span class="shrink-0 text-xs text-neutral-500">{formatTime(item.created_at)}</span>
      </div>

      <p class="mt-1 text-sm text-neutral-700">{item.content}</p>
        ...
        <div class="mt-3 flex flex-wrap gap-2">
          {#each item.options as option}
            <button
              ...
              class="rounded-md bg-neutral-900 px-3 py-1.5 text-sm font-medium text-white transition-colors hover:bg-neutral-700 disabled:opacity-50"
            >
              {option}
            </button>
          {/each}
      ...
      {#if item.item_type === 'question' && item.status === 'pending'}
        <form onsubmit={(e) => { e.preventDefault(); handleCustomSubmit(); }} class="mt-3">
          <div class="flex gap-2">
            <input
              ...
              disabled={isResponding}
              class="flex-1 rounded-md border border-neutral-300 px-3 py-1.5 text-sm focus:border-neutral-500 focus:outline-none disabled:opacity-50"
            />
            <button
              type="submit"
      ...
      {#if item.status === 'responded' && item.response}
        <div class="mt-2 rounded bg-neutral-100 px-2 py-1 text-sm text-neutral-600">
