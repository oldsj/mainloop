<script lang="ts">
  ...
  const sessionsTabClass = $derived(
    $mobileTab === 'sessions' ? 'text-term-accent' : 'text-term-fg-muted'
  );
...
>
  <div class="flex items-center justify-around">
    ...
    <button
      ...
      onclick={() => mobileTab.set('sessions')}
      class="flex flex-1 flex-col items-center gap-1 py-3 transition-colors {sessionsTabClass}"
      aria-label="Sessions"
      aria-pressed={$mobileTab === 'sessions'}
    >
      <svg
        xmlns="http://www.w3.org/2000/svg"
        fill="none"
